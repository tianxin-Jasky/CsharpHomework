...
namespace program6_1
{
    class Program
    {
        static void Main(string[] args)
        {
                    ...
                    case 5:
                        ...
                        switch (qqq)
                        {
                            case 1:
                                ...
                                order3.modifyOrderName(SearchName1, SearchName2);
                                break;
                            case 2:
                                ...
                                order3.modifyOrderNum(Searchnum, Searchnum2);
                                break;
                        }
                        break;
                    case 6:
            ...
            String xmlFireName = "s.xml";
            Export(orderlist,xmlFireName);
            Import(xmlFireName);
            Environment.Exit(0);
        }

        //反序列化q并输出在控制台上
        public static void Import(String xmlFireName)
        {
            XmlSerializer xmlser = new XmlSerializer(typeof(List<Order>));
           
            List<Order> twoList = XmlDeserialize(xmlser, xmlFireName);
            OrderService order111 = new OrderService();
            foreach (Order qqqq in twoList)
            {
                order111.PrintOrder(qqqq);
            }
        }
        public static List<Order> XmlDeserialize(XmlSerializer ser,string filename)
        {
            ...
            List<Order> one = ser.Deserialize(tr) as List<Order>;
            tr.Close();
            return one;
        }

        //XML序列化和显示
        public static void Export(List<Order> i, String xmlFireName)
        {
            XmlSerializer xmlser = new XmlSerializer(typeof(List<Order>));
            XmlSerialize(xmlser, xmlFireName, i);
            string xml = File.ReadAllText(xmlFireName);
...
public class OrderService
{
    ...
    {
        if(i!=null) Console.WriteLine(i.ClientName + "  订单号：" + i.Number + "  苹果的个数" + i.AppleNum + "  球的个数" + i.BallNum + "  笔的个数" + i.PenNum + " 订单总金额为:" + i.price);
